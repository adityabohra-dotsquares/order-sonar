...
from models.orders import OrderStatus, Order, OrderItem, OrderItemTracking, OrderDetails, OrderReturn, PaymentStatus
...
VALID_SORT_FIELDS = {
    "created_at", "updated_at", "status", "payment_status", "total_amount",
    "order_number", "warehouse_id", "courier", "tracking_number",
    "actual_delivery_date", "items_count", "total_saving", "shipped_at"
}
...
PRODUCT_SERVICE_URL = os.getenv("PRODUCT_URL", "https://shopper-beats-products-877627218975.australia-southeast2.run.app")
...
async def handle_inventory(
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to contact inventory service: {str(e)}")
...
async def update_order_status_logic(
    ...
    if new_status not in [OrderStatus.PARTIALLY_SHIPPED, OrderStatus.PARTIALLY_RETURNED, OrderStatus.PARTIALLY_REFUNDED]:
    ...
    if new_status == OrderStatus.UNSHIPPED and old_status not in [OrderStatus.PENDING, OrderStatus.CONFIRMED, OrderStatus.PROCESSING]:
        ...
        if courier:
            ...
            cp_result = await db.execute(select(ShipmentPartner).where(ShipmentPartner.name.ilike(courier)))
    ...
    elif new_status in [OrderStatus.REPLACEMENT, OrderStatus.RETURN_REQUESTED, OrderStatus.REPLACEMENT_REQUESTED]:
...
async def call_payment_refund(payload: Dict[str, Any], idempotency_key: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    for order in orders:
        ...
        for item in order.items:
            ...
            rows.append({
                ...
                "Sale Price": None,
                "Shipping Price": float(order.shipping_cost) if order.shipping_cost else None,
                "Total Price": float(order.total_amount) if order.total_amount else None,
                "Item Weight": None,
                ...
                # Payment
                "Payment Details": order.payment_status.value if order.payment_status else None,
...
async def create_replacement_order(db: AsyncSession, original_order: Order, return_req: OrderReturn):
    """
    ...
    for ret_item in return_req.items:
        ...
        orig_item = next((i for i in original_order.items if i.id == ret_item.order_item_id or i.product_id == ret_item.product_id), None)
...
async def update_order_address(
    ...
    address_config = {
        "shipping": {
            "data": shipping_addr,
            "fields": ["first_name", "last_name", "company", "address", "apartment", "city", "state", "country", "postal_code", "phone", "house_no", "landmark"]
        },
        "billing": {
            "data": billing_addr,
            "fields": ["first_name", "last_name", "company", "address", "apartment", "city", "state", "country", "postal_code", "phone", "house_no"]
        }
    }
